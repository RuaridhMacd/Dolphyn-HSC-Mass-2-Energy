...
hsc_trucks_changes = Dict(
    ...
    "Inv_Cost_Energy_p_tonne_yr" => ("Inv_Cost_Energy_p_MW_yr", 1/HHV_H2),
    "Fixed_OM_Cost_Energy_p_tonne_yr" => ("Fixed_OM_Cost_Energy_p_MW_yr", 1/HHV_H2),
...
function update_file(case, file, hsc_changes)
...
for case in cases
    ...
    for file in case_files
        update_file(case, file, hsc_changes)
    end
    ...
    if isdir(tdr_path)
        ...
        for file in tdr_files
            update_file(joinpath(case, tdr_name), file, hsc_changes)
        end
    end
